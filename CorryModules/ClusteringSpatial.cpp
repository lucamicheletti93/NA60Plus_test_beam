...
ClusteringSpatial::ClusteringSpatial(Configuration& config, std::shared_ptr<Detector> detector)
    ...
    config_.setDefault<bool>("use_larger_cluster", false);
    config_.setDefault<int>("min_cluster_size",0);
    config_.setDefault<int>("max_cluster_size",200);
    ...
    onlyLargerCluster = config_.get<bool>("use_larger_cluster");
    min_cluster_size = config_.get<int>("min_cluster_size");
    max_cluster_size = config_.get<int>("max_cluster_size");
}
...
void ClusteringSpatial::initialize() {
    ...
    clusterSize = new TH1F("clusterSize", title.c_str(), 200, -0.5, 199.5);
...
StatusCode ClusteringSpatial::run(const std::shared_ptr<Clipboard>& clipboard) {
    ...
    double current_cluster = 0;
    auto max_cluster = std::make_shared<Cluster>();
    ...
    for(auto pixel : pixels) {
        ...
        if(static_cast<double>(cluster->size())<min_cluster_size)
            continue;
        if(static_cast<double>(cluster->size())>max_cluster_size)
            continue;
        // Fill cluster histograms
        LOG(DEBUG) << "cluster local: " << cluster->local();
        if(onlyLargerCluster){
            if(static_cast<double>(cluster->size())>current_cluster){
                max_cluster = cluster;
                hasCluster = true;
                current_cluster = static_cast<double>(cluster->size());
                LOG(DEBUG) << "new largest: " << static_cast<double>(cluster->size());
            }
        }
        else{
    ...
    if(onlyLargerCluster && hasCluster){
        deviceClusters.push_back(max_cluster);
        clusterSize->Fill(static_cast<double>(max_cluster->size()));
        clusterWidthRow->Fill(static_cast<double>(max_cluster->rowWidth()));
        clusterWidthColumn->Fill(static_cast<double>(max_cluster->columnWidth()));
        clusterCharge->Fill(max_cluster->charge());
        clusterSeedCharge->Fill(max_cluster->getSeedPixel()->charge());
        clusterPositionGlobal->Fill(max_cluster->global().x(), max_cluster->global().y());
        clusterPositionLocal->Fill(max_cluster->column(), max_cluster->row());
        clusterTimes->Fill(static_cast<double>(Units::convert(max_cluster->timestamp(), "ns")));
    }
